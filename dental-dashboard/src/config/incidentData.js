export const incidentData = [
  {
    "id": "i1",
    "patientId": "p1",
    "title": "Toothache",
    "description": "Upper molar pain",
    "comments": "Sensitive to cold",
    "appointmentDate": "2025-07-01T10:00:00",
    "cost": 80,
    "status": "Completed",
    "files": [
      { 
        "name": "invoice.pdf", 
        "url": "data:application/pdf;base64,", 
        "type": "application/pdf" 
      },
      { 
        "name": "xray.png", 
        "url": "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==", 
        "type": "image/png" 
      }
    ]
  },
  {
    "id": "i2",
    "patientId": "p3",
    "title": "Routine Dental Checkup",
    "description": "Annual dental cleaning and inspection",
    "comments": "Plaque buildup noted",
    "appointmentDate": "2025-06-28T09:30:00",
    "cost": 50,
    "status": "Completed",
    "files": []
  },
  {
    "id": "i3",
    "patientId": "p4",
    "title": "Cavity Filling",
    "description": "Cavity in lower right molar",
    "comments": "Used composite material",
    "appointmentDate": "2025-06-25T14:00:00",
    "cost": 120,
    "status": "Completed",
    "files": [
      { 
        "name": "before.jpg", 
        "url": "data:image/jpeg;base64,", 
        "type": "image/jpeg" 
      }
    ]
  },
  {
    "id": "i5",
    "patientId": "p2",
    "title": "Wisdom Tooth Extraction",
    "description": "Painful impacted wisdom tooth",
    "comments": "Surgery successful, follow-up in 7 days",
    "appointmentDate": "2025-06-18T16:30:00",
    "cost": 40,
    "status": "Completed",
    "files": [
      { 
        "name": "xray-wisdom.jpg", 
        "url": "data:image/jpeg;base64,", 
        "type": "image/jpeg" 
      }
    ]
  },
  {
    "id": "i6",
    "patientId": "p5",
    "title": "Dental X-Ray",
    "description": "Full mouth x-ray before braces",
    "comments": "Awaiting orthodontist plan",
    "appointmentDate": "2025-07-26T10:45:00",
    "cost": 100,
    "status": "Pending",
    "files": []
  },
  {
    "id": "i7",
    "patientId": "p8",
    "title": "Root Canal Consultation",
    "description": "Pain in lower left molar, possible root canal",
    "comments": "MRI not needed, awaiting confirmation",
    "appointmentDate": "2025-07-24T15:00:00",
    "cost": 150,
    "status": "In Progress",
    "files": [
      { 
        "name": "tooth-xray.pdf", 
        "url": "data:application/pdf;base64,", 
        "type": "application/pdf" 
      }
    ]
  },
  {
    "id": "i8",
    "patientId": "p10",
    "title": "Tooth Cleaning",
    "description": "Dental plaque removal",
    "comments": "Suggested every 6 months",
    "appointmentDate": "2025-05-30T13:00:00",
    "cost": 70,
    "status": "Completed",
    "files": []
  },
  {
    "id": "i9",
    "patientId": "p11",
    "title": "RCT",
    "description": "Recurring toothaches",
    "comments": "MRI scan scheduled",
    "appointmentDate": "2025-08-20T09:00:00",
    "cost": 90,
    "status": "Scheduled",
    "files": []
  },
  {
    "id": "i10",
    "patientId": "p7",
    "title": "Dental Trauma",
    "description": "Cracked incisor from fall",
    "comments": "Temporary bonding done",
    "appointmentDate": "2025-05-20T17:45:00",
    "cost": 65,
    "status": "Completed",
    "files": [
      { 
        "name": "xray-incisor.png", 
        "url": "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==", 
        "type": "image/png" 
      }
    ]
  }
]